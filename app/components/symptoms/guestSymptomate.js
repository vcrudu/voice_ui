...
class PatientSymptomateResult extends React.Component {
    ...
    render() {
        ...
        if (diagnosticResult && diagnosticResult.conditions) {
            sortedConditions = diagnosticResult.conditions.sort((condition1, condition2) => {
        ...
        const conditionHtml = sortedConditions && sortedConditions.length>0 ? <b>{sortedConditions[0].name}</b>:null;
        const notSignedInText = !this.props.userData?'If you enroll to our service, we can make the appointment on your behalf and be able to provide you with more support options':'';
        return this.props.show ? (<div>
            ...
            <Card style={{maxHeight:'42vh',overflowY:'scroll'}}>
                {coditions}
            </Card>
            <Typography style={{width:'80vw',marginLeft:'auto', marginTop:'10px', marginRight:'auto',marginBottom:'10px',textAlign:'center'}} tag="h3">RECOMMENDATION</Typography>
            <Card style={{ width: '21rem', marginTop: '5px', marginLeft: 'auto', marginRight: 'auto' }}>
                            <CardPrimaryAction>
                                <div style={{ padding: '1rem 1rem 1rem 1rem' }}>
                                    <Typography use="body2" tag="div" theme="text-secondary-on-background">We have the results of your Health Check. Our preliminary assessment is that you may have {conditionHtml} and suggest to make an appointment with your clinician. {notSignedInText}</Typography>
                                </div>
                            </CardPrimaryAction>
                            <CardActions>
                                <CardActionButtons>
                                   
                                    {
                                        this.props.userData?
                                        (
                                            <Link to='/stage/appointments'>
                                                <CardAction>Book appointment</CardAction>
                                            </Link>
                                        ): 
                                        (
                                            <Link to='/signup'>
...
class GuestSymptomateComponent extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        if(this.state.dateOfBirth || (this.state.age && this.state.sex))
    ...
    handlePrev() {
        if (this.state.selectionStep > 0) {
            ...
            if (step == 0) {
                this.setState({question:null, diagnosticResponse:{}});
    ...
    handleCommonSymptoms(){
        let emptyEvidence = { "sex": this.state.sex.toLowerCase(), "age": this.state.age, "evidence": [] };
        symptomateService.getCommonSymptoms({"text": this.state.symptomText},(error, result) => {
            symptomateService.getSymptoms((error, symptomsResult) => {
                if (symptomsResult.length > 0) {
                    var step = this.state.selectionStep + 1;
                    this.setState({ commonSymptoms: symptomsResult, 
                        showCommonSymptoms: true, 
                        emptyEvidence: emptyEvidence, 
                        diagnostic: emptyEvidence,
                        noCommonSymptoms: false
                     });
                }
            });
        })
    ...
    handleCommonSymptomsBack(){
        this.setState({ showCommonSymptoms: false, age:null});
    }

    handleCommonBack(){
        this.setState({noCommonSymptoms:true});
    }

    onSymptomTextChange(event) {
        if(event.target.value.length>=10){
            this.setState({ nextDisabled: false });
        }
        this.setState({ symptomText: event.target.value });
    }

    render() {
        return this.state.age || this.state.showCommonSymptoms
            ?(<Grid>
            <GridCell span="4">
                <div>
                    {
                    ...
                    <PatientSymptomateResult show={this.state.showResult} diagnosticResult={this.state.diagnosticResponse} diagnosticPost={this.state.diagnostic} slotId={this.state.slotId} userData={this.props.userData} /> 
                </div>
            </GridCell>
        </Grid>):
            (
...
const GuestSymptomate = connect(
    mapStateToProps,
    mapDispatchToProps
)(GuestSymptomateComponent);

export default GuestSymptomate;