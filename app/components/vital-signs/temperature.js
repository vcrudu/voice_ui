...
class VitalSignChart extends React.Component {
    ...
    drawGraphic(props) {
        ...
        if (props.type != "bloodPressure") {
            ...
            for (var i = 0; i < props.dataSource.values.length; i++) {
                tempArray1.push({
                    ...
                    color: props.dataSource.values[i].value>37? 'red': "#7E57C2",
        ...
        this.yScale = d3.scaleLinear().domain([20,50]).range([height-this.y_margin,this.y_margin]);
        ...
        this.valueTexts = svg.selectAll("circle")
            ...
            .style("fill", "red")
            .style("font-size", 10)
            .text(d=>d.value + ' '+String.fromCharCode(0x00b0));
...
class WeightComponent extends React.Component {
    ...
    onFabClick() {
        ...
        this.props.actions.setCurrentChatCommand({ message: "Let's measure your temperature", measureType: 'temperature' });
    }
    ...
    componentDidMount() {
        ...
        this.props.actions.changeScreenTitle('Temperature');
    }

    render() {
        if (!this.state.temperatureVitalSignsDef) return null;
        if (this.state.redirect) {
            const chatUrl = `/chat/temperature/${'MeasureTemperature'}/${moment().valueOf()}/Temperature`;
            const voiceUrl = `/voice/temperature/${'MeasureTemperature'}/${moment().valueOf()}/Temperature}`;
            let url = this.props.voice.voiceState === 'off' ? chatUrl : voiceUrl;
            return <Redirect to={url} />
        } else
            return (
                <div>
                    <LeafHeader backUrl='/stage/signs' title='Temperature' />
                    ...
                    ></Fab>
                <VitalSignChart dataSource={this.state.temperatureVitalSignsDef}
                    aspectWidth={16}
                    aspectHeight={9}
                    ticks={10}
                    mobileThreshold={500}
