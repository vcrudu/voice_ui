...
class VitalSignChart extends React.Component {
    ...
    drawGraphic(props) {
        ...
        this.yScale = d3.scaleLinear().domain([0,200]).range([height-this.y_margin,this.y_margin]);
...
class WeightComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        this.props.actions.changeScreenTitle('Weight');
    }

    render() {
        if (!this.state.weightDef) return null;
        if (this.state.redirect) {
            ...
            return (
                <div>
                    ...
                    ></Fab>
                <VitalSignChart dataSource={this.state.weightDef}
                    aspectWidth={16}
                    aspectHeight={9}
                    ticks={10}
                    mobileThreshold={500}
